...
**Durability model (O2):** the **fsync'd append-only journal is the durability source of truth**; the published key blob is a **rebuildable materialized view** of it. A write returns `OK` only after its journal record is durably persisted (**append + `FlushFileBuffers` on an already-open journal file** — an append to an existing directory entry, well-defined on Windows). The atomic publish (`os.replace`) provides **reader-consistency**, not durability. **On resume, the journal is replayed to re-materialize any key whose published copy was lost or torn.** Remote backends derive durability from their server ACK; git from the ref update.
...
- current == expected_hash → the store still holds the **pre-write** value, so our write **did not land**. Run the caller's single retry (if provided) and **return its real result unchanged** (OK / STALE / EXISTS / ERROR): OK = the value is now set; a STALE = the retry genuinely lost a fresh race and the caller's value is not current. This is a **true** STALE, not a false one — the original's non-commit is established by current==expected. *(Refined v1.5, driven by the independent adversarial code review: the earlier "collapse a non-OK retry to CONFLICT_UNKNOWN" discarded actionable truth; `intended_new_hash` must also be a valid 64-hex — never None — so a None==None coincidence can't report a false OK.)*
- **otherwise (superseded / indeterminate) → `CONFLICT_UNKNOWN` (still outcome-unknown), NEVER `STALE`.**

Under content-hash CAS a lost ack cannot distinguish "our write committed then was superseded" from "our write never committed," so the contract must **not** assert definitely-not-committed. A **native monotonic generation resolves the original write's fate ONLY if the writer captured its own write's target generation before the ack was lost**; a merely-*current* generation identifies the current version, **not our write's history, and MUST NOT be used to infer non-commit**. When our own generation was not captured, the result stays `CONFLICT_UNKNOWN`. Callers on content-hash-only paths must be **idempotent** and tolerate `CONFLICT_UNKNOWN`.
...
CAS-reject (incl. races) · atomicity + crash leaves nothing torn/unscanned at the key · **journal-replay durability** · secret scan incl. side paths · no-egress · lock TTL+token · uncertain-outcome reconcile (committed-then-superseded → CONFLICT_UNKNOWN, never false STALE) · **Windows two-process on NTFS** · migration.
